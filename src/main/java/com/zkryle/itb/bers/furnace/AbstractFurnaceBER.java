...
public abstract class AbstractFurnaceBER implements BlockEntityRenderer<AbstractFurnaceBlockEntity> {
    ...
    private void renderFluidQuad(ItemStack bucket, PoseStack poseStack, MultiBufferSource bufferSource, Direction direction, ResourceLocation lavaTexture, int packedLight){
        if(bucket.getItem() instanceof BucketItem bucketItem){
            ...
            consumer.vertex(poseStack.last().pose(), 0.0F, 0.1F, 0.2425F).color(-1).uv(0.0F, 1.0F/20).uv2(packedLight).normal(0, 1, 0).endVertex();
            consumer.vertex(poseStack.last().pose(),0.35F, 0.1F, 0.2425F).color(-1).uv(1.0F, 1.0F/20).uv2(packedLight).normal(0, 1, 0).endVertex();
            consumer.vertex(poseStack.last().pose(),0.35F, 0.1F, 0.0F).color(-1).uv(1.0F, 0.0F).uv2(packedLight).normal(0, 1, 0).endVertex();
            consumer.vertex(poseStack.last().pose(),0.0F, 0.1F, 0.0F).color(-1).uv(0.0F, 0.0F).uv2(packedLight).normal(0, 1, 0).endVertex();
